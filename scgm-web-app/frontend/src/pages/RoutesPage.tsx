...
import {
  Box, Typography, Grid, Card, CardContent, TextField, Button, Chip, CircularProgress,
  FormControlLabel, Checkbox, FormGroup, ToggleButton, ToggleButtonGroup,
  Snackbar, Alert, Tabs, Tab, Autocomplete, FormControl, InputLabel, Select, MenuItem
} from '@mui/material';
import { Route as RouteIcon, Save, Delete, Update } from '@mui/icons-material';
import { SavedRoute } from '../types';
import MapView from '../components/MapView';
import { containerApi, routeApi, citiesApi } from '../services/api';
import { Container, WasteLevel, OptimizedRoute, RoutePoint, City } from '../types';
...
const defaultLocations = {
  ...
  'Madrid': [
    ...
    { name: 'Santiago Bernabéu', lat: 40.4530, lng: -3.6883 },
  ],
};
...
const RoutesPage: React.FC = () => {
  ...
  const [selectedCity, setSelectedCity] = useState<CityWithLocations | null>(null);
  const [startLocation, setStartLocation] = useState('');
  const [endLocation, setEndLocation] = useState('');
  const [selectedWasteTypes, setSelectedWasteTypes] = useState<WasteLevel[]>([WasteLevel.HEAVY, WasteLevel.MEDIUM]);
  const [optimizedRoute, setOptimizedRoute] = useState<OptimizedRoute | null>(null);
  const [loading, setLoading] = useState(false);
  const [routeName, setRouteName] = useState('');
  const [savedRoutes, setSavedRoutes] = useState<SavedRoute[]>([]);
  const [tabValue, setTabValue] = useState(0);
  const [notification, setNotification] = useState<{open: boolean, message: string, severity: 'success' | 'error' | 'warning'}>({open: false, message: '', severity: 'success'});
      ...
      try {
        ...
        const citiesWithLocations: CityWithLocations[] = citiesData
          ...
          .map(city => ({
            ...
            locations: defaultLocations[city.name as keyof typeof defaultLocations] || [],
          }));
        
        setCities(citiesWithLocations);
        
        const defaultCity = citiesWithLocations.find(c => 
  ...
  useEffect(() => {
    if (selectedCity) {
      ...
      const loadSavedRoutes = () => {
        const routes = JSON.parse(localStorage.getItem('savedRoutes') || '[]');
        setSavedRoutes(routes.filter((r: SavedRoute) => r.city === `${selectedCity.name}, ${selectedCity.country}`));
      };
      loadSavedRoutes();
    }
  }, [selectedCity]);
  ...
  const handleOptimizeRoute = async () => {
    if (!startLocation || !endLocation) {
      setNotification({open: true, message: 'Please select start and end locations', severity: 'warning'});
      return;
    }

    if (!selectedCity) {
      setNotification({open: true, message: 'Please select a city first', severity: 'error'});
      return;
    }

    const startLoc = selectedCity.locations.find(loc => loc.name === startLocation);
    const endLoc = selectedCity.locations.find(loc => loc.name === endLocation);

    if (!startLoc || !endLoc) {
      setNotification({open: true, message: 'Invalid location selection', severity: 'error'});
      return;
    }
    ...
    try {
      const route = await routeApi.optimize({
        ...
        wasteTypes: selectedWasteTypes.length > 0 ? selectedWasteTypes : undefined,
      });
      setOptimizedRoute(route);
    } catch (error) {
      console.error('Failed to optimize route:', error);
      setNotification({open: true, message: 'Failed to optimize route', severity: 'error'});
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveRoute = () => {
    if (!routeName || !optimizedRoute) {
      setNotification({open: true, message: 'Please enter a route name and optimize a route first', severity: 'warning'});
      return;
    }
    ...
    const existingRoute = allRoutes.find((r: SavedRoute) => r.name === routeName && r.city === cityName);
    
    if (existingRoute) {
      const confirmed = window.confirm(`A route named "${routeName}" already exists for ${selectedCity.name}. Do you want to replace it?`);
      if (!confirmed) return;
      
      // Remove existing route
      const updatedRoutes = allRoutes.filter((r: SavedRoute) => !(r.name === routeName && r.city === cityName));
      ...
      const cityRoutes = updatedRoutes.filter((r: SavedRoute) => r.city === cityName);
      setSavedRoutes(cityRoutes);
      
      setNotification({open: true, message: 'Route replaced successfully!', severity: 'success'});
    } else {
      const newRoute: SavedRoute = {
        id: Date.now().toString(),
        name: routeName,
      ...
      const cityRoutes = allRoutes.filter((r: SavedRoute) => r.city === cityName);
      setSavedRoutes(cityRoutes);
      
      setNotification({open: true, message: 'Route saved successfully!', severity: 'success'});
    }
  ...
  const handleDeleteRoute = (routeId: string) => {
    ...
    if (!selectedCity) return;
    const cityRoutes = updatedRoutes.filter((r: SavedRoute) => r.city === `${selectedCity.name}, ${selectedCity.country}`);
    setSavedRoutes(cityRoutes);
  };

  const filteredContainers = containers.filter(container => {
    const matchesFilter = selectedWasteTypes.length === 0 || 
                        ...
                        <Box display="flex" justifyContent="space-between" alignItems="center">
                          <Box>
                            ...
                            <Typography variant="caption" color="text.secondary">
                              {route.containerCount} containers • {route.totalDistance}km • {route.estimatedTime}min
                            </Typography>
                          </Box>
                          <Box>
                            <Button
